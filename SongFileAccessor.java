...
public class SongFileAccessor extends FileAccessor{
  ...
  */
  public void processLine(String line){
   ...
   if (lineArr[1].equals("")) {
      lineSong = new Song(lineArr[0], lineArr[2], Double.parseDouble(lineArr[3]));
   }
   else {
      lineSong = new Song(lineArr[0], lineArr[1], lineArr[2], Double.parseDouble(lineArr[3]));
   }
   songs.addSong(lineSong);
  }
  ...
  */   
   public String songToCSVString(Song song){
      String csv = song.getTitle() + "," + song.getAlbum() + "," + song.getArtist() + "," + song.getPlayTime();      
      return csv;
   }
  ...
  */
  public String getSongsAsCSV(SongList songList){
      ...
      for (int i = 0; i < songList.getSongList().size(); i++) {
         songListCSV += songToCSVString(songList.getSongList().get(i));
         songListCSV += "\n";
      }
      return songListCSV;
   }
